...
const float PLANET_TURN_RATE = 0.015f;
const float VECH_TURN_RATE = -0.0015f;
const float ROCK_TURN_RATE = 0.005f;


int SpaceCraft;
int Planet1, Planet2;
int VechStart, VechEnd;
int AlienStart, AlienEnd;
int ChickenStart, ChickenEnd;
...
void keyboard(unsigned char key, int x, int y)
{
	...
	if (key == 27) { // escape key to close the program
		exit(0);
	}
	else if (key == 'd') {
...
void loadtexture()
{
	textureID[1] = loadBMP_custom("resources\\texture\\leisure_spacecraftTexture.bmp");
	textureID[2] = loadBMP_custom("resources\\texture\\spacecraftTexture.bmp");
	textureID[3] = loadBMP_custom("resources\\texture\\colorful_alien_vehicleTexture.bmp");
	textureID[4] = loadBMP_custom("resources\\texture\\RockTexture.bmp");
	textureID[5] = loadBMP_custom("resources\\texture\\planetTexture.bmp");
	textureID[6] = loadBMP_custom("resources\\texture\\chickenTexture.bmp");
	textureID[7] = loadBMP_custom("resources\\texture\\alienTexture.bmp");
	textureID[8] = loadBMP_custom("resources\\texture\\colorful_alien_vehicleTexture.bmp");

	cubeTextureID[0] = loadBMP_custom("resources\\texture\\universe skybox\\right.bmp");
	cubeTextureID[1] = loadBMP_custom("resources\\texture\\universe skybox\\left.bmp");
	cubeTextureID[2] = loadBMP_custom("resources\\texture\\universe skybox\\top.bmp");
	cubeTextureID[3] = loadBMP_custom("resources\\texture\\universe skybox\\bottom.bmp");
	cubeTextureID[4] = loadBMP_custom("resources\\texture\\universe skybox\\front.bmp");
	cubeTextureID[5] = loadBMP_custom("resources\\texture\\universe skybox\\back.bmp");
}

void loadObj()
{

	bufferObject(0, "resources\\chicken.obj");
	bufferObject(1, "resources\\spaceCraft.obj");
	bufferObject(2, "resources\\alienpeople.obj");
	bufferObject(3, "resources\\rock.obj");
	bufferObject(4, "resources\\planetCentered.obj");
	bufferObject(5, "resources\\alienvehicle.obj");
	bufferObject_reverse_normal(6, "resources\\planet.obj");
}
...
void paintGL(void)
{
	...
	for (int i = VechStart; i <= VechEnd; i++) {
		EntityList[i]->transform = glm::rotate(mat4(), VECH_TURN_RATE, glm::vec3(0.0f, 1.0f, 0.0f)) *  EntityList[i]->transform;
	}
...
void renderSkybox()
{
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, cubeTextureID[2]);
	glUniform1i(localTextureID, 0);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, cubeTextureID[3]);
	glUniform1i(localTextureID, 0);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, cubeTextureID[4]);
	glUniform1i(localTextureID, 0);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, cubeTextureID[5]);
	glUniform1i(localTextureID, 0);
...
void initialiseEntities() {
	...
	Planet2 = initEntity(4, 5, +300, 10, 0, 23.0f, 2); // the wonder planet
	...
	VechStart = entityCount;//initialise all the rings
	for (int i = 0; i < 3; i++) {
		VechEnd = initEntity(5, 7, 60 + 30 * i, +10, -60 ,  glm::rotate(mat4(), glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f)), 2.0f, 3);
	}
	AlienStart = entityCount;
	for (int i = 0; i < 3; i++) {
		AlienEnd = initEntity(2, 7, 60 + 30 * i, +13, -60, glm::rotate(mat4(), glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f)), 2.0f, 3);
	}

	ChickenStart = entityCount;
	for (int i = 0; i < 3; i++) {
