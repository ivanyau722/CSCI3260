...
/*
char loadBMP_custom(const char* imagepath, unsigned char* data, int& width, int& height) {
...
/*unsigned int loadCubemap(std::vector<const GLchar*> faces)
{
	...
	for (GLuint i = 0; i < faces.size(); i++)
	{
		loadBMP_custom(faces[i], data, width, height);
	}
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
...
void initializedGL(void) //run only once
{
	...
	Shader shader;
	shader.setupShader("6.1.cubemaps.vs", "6.1.cubemaps.fs");
	Shader skyboxShader;
	skyboxShader.setupShader("6.1.skybox.vs", "6.1.skybox.fs");
...
{
	glClearColor(1.0f, 1.0f, 1.0f, 1.0f); //specify the background color, this is just an example
...
int main(int argc, char* argv[])
{
	...
	window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "CSCI3260Project", NULL, NULL);
	if (!window) {
